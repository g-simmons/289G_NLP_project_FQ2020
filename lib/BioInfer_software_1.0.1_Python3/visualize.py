...
class DepView(Frame):
    ...
    def drawEntities(self,sentence):
        ...
        for e in sentence.entities:
            ...
            ordered_st = sorted(ordered_st, key=lambda x: x.sequence)
    ...
    def drawLinks(self, sentence):
        ...
        if len(list(sentence.linkages.keys()))==0:
            #print >> sys.stderr, "No linkage found for sentence", sentence
            return
        else:
            self.default_link_type = list(sentence.linkages.keys())[0]
            for k,v in list(sentence.linkages.items()):
                ...
                ordered_links = sorted(ordered_links, key=lambda x: abs(x.token1.sequence - x.token2.sequence))
    ...
    def highlight(self, tags):
        ...
        tags = sorted(tags, key = functools.cmp_to_key(bbox_comp))
...
class EntView(TextAsListbox):
    ...
    def draw(self,sentence):
        ...
        self.clear()
        self.list.config(state="normal")
        for l,(k,v) in enumerate((k,v) for (k,v) in list(sentence.entitiesById.items()) if not v.isFormulaRelationship()):
...
class RelView(Frame):
    ...
    def ascend(self,event=None):
        ...
        if len(sel)==1 and 'expanded' in self.canvas.gettags(sel[0]):
            ...
            items = [a for a in [i for i in self.canvas.find_withtag(ALL)] if a>sel]
...
class Visualiser(Tk):
    ...
    def showSentenceByIndex(self,idx):
        ...
        self.dependencybar.setTypes(list(self.parser.bioinfer.sentences.sentences[idx].linkages.keys()))
