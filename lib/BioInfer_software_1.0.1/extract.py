...
def printTokenOffsets(sentence):
    ...
    for i, t in enumerate(sentence.tokens):
        ...
        print "token(%d, [%d-%d], '%s')" % (i, from_off, to_off, text),
...
def printLinkageDependencies(sentence, linkage):
    ...
    if not sentence.linkages.has_key(linkage):
        print >> sys.stderr, "missing '%s' linkage for sentece %s" % (linkage, sentence.id)
        sys.exit(1)
    ...
    for l in links:
        ...
        print "%s-%s%s" % (l.token1.sequence, l.token2.sequence, type_string),
...
def printEntities(sentence):
    ...
    entities = [e for e in sentence.entitiesById.values() if not e.isFormulaRelationship()]

    for e in entities:
        offset_text = ",".join(map(lambda a: "%d-%d" % a, e.getCharOffsets()))
        print "%s(%s, [%s], '%s')" % (e.type.name, e.id, offset_text, e.getText()),
...
def relationshipToString(r):
    ...
    s = "%s([%s], " % (r.predicate.name, ",".join(map(lambda a: "%d-%d" % a, st_offsets)))
...
if __name__=="__main__":
    ...
    if not options.bioinferXmlFile:
        print >> sys.stderr, "You must specify the --bioInferFile (-b) option."
    ...
    if len([a[0] for a in actions if options.__dict__[a[0]]]) == 0:
        print >> sys.stderr, "Please specify at least one extraction option."
        optionParser.print_help()
        sys.exit(1)
