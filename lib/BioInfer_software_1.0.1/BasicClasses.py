...
class BIXMLWriteable (object):
    ...
    def writeXMLOpen(self,out,indent=0,closing=False):
        ...
        if closing:
            ...
            closeChar=""
        print >> out, " "*indent+"<%s>"%(" ".join((self.XMLTag,self.XMLAttrs())).strip()+closeChar)
    ...
    def writeXMLClose(self,out,indent=0):
        ...
        @type indent: Integer
        """
        print >> out, " "*indent+"</%s>"%self.XMLTag
...
class Sentence (BIObject,BIXMLWriteable):
    ...
    def writeXMLNestedItems(self,out,indent):
        ...
        print >> out, " "*indent+"<linkages>"
        for (t,l) in self.linkages.items():
            l.writeXML(out,indent+DEFAULT_INDENT)
        print >> out, " "*indent+"</linkages>"
        print >> out, " "*indent+"<formulas>"
...
class SubToken (BIObject,BIXMLWriteable):
    ...
    def getCharOffset(self):
        ...
        for st in filter(lambda st: st.sequence<self.sequence, self.token.subTokens):
            offset += len(st.text)
        return offset
...
class Entity (BIObject,BIXMLWriteable):
    ...
    def writeXMLNestedItems(self,out,indent):
        for s in self.subTokens:
            print >> out, " "*indent+"<nestedsubtoken id=%s/>"%xml.sax.saxutils.quoteattr(s.id)

    def writeXMLNestedEntities(self,out,indent):
        for e in self.nestedEntities:
            print >> out, " "*indent+"<entitynesting outerid=%s innerid=%s/>"%(xml.sax.saxutils.quoteattr(self.id),xml.sax.saxutils.quoteattr(e.id))
