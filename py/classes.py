...
class BioInferTaskConfiguration():
    ...
    def _split_schema_keys(self, schema):
        split_schema = {}
        for predicate, argset in schema.items():
            split_schema[predicate] = {tuple(k.split(' ')):v for k,v in argset.items()}
    ...
    def from_json(self, path, entity_prefix: str = ENTITY_PREFIX, predicate_prefix: str = PREDICATE_PREFIX) -> None:
    ...
    def from_parser(self, parser: BIParser, entity_prefix: str = ENTITY_PREFIX, predicate_prefix: str = PREDICATE_PREFIX) -> None:
        self.entity_prefix = entity_prefix
        self.predicate_prefix = predicate_prefix
